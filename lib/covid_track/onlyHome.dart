...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() async{
    super.initState();
    var result = await this.getStatesData();

    stateWise = result[0];
    dailyStateWise = result[1];
    tActive = int.parse(stateWise[0]['active']);
    tDeaths = int.parse(stateWise[0]['deaths']);
    tConfirmed = int.parse(stateWise[0]['confirmed']);
    tRecovered = int.parse(stateWise[0]['recovered']);
    getDistrictData();
  }
  ...
  Future getStatesData() async {
    ...
    // }));
    return [stateWise, dailyStateWise];
  }
                ...
                Row(
                  ...
                  children: <Widget>[
                    BoxDisplay(
                        color: Colors.red,
                        title: 'Active',
                        text: stateWise[0]['active']),
                    BoxDisplay(
                        color: Colors.orange,
                        title: 'Confirmed',
                        text: stateWise[0]['confirmed']),
                  ],
                ),
                SizedBox(
                  height: 5,
                ...
                Row(
                  ...
                  children: <Widget>[
                    BoxDisplay(
                        color: Colors.blue,
                        title: 'Deaths',
                        text: stateWise[0]['deaths']),
                    BoxDisplay(
                        color: Colors.green,
                        title: 'Recovered',
                        text: stateWise[0]['recovered']),
                  ],
                ),
              ],
            ),
